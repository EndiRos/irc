...
/*   Updated: 2025/10/30 10:25:19 by enetxeba         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void mode(std::string &msg, User& user, std::map<std::string, Channel> &channels_list)
{
	...
	while(msg[pos] && msg[pos] != ' ')
	{
		...
		{
			case 'i':
				res.user += execute_i(mode, channels_list[channel]);
				break;
			case 't':
				res.user += execute_t(mode, channels_list[channel]);
				break;
			case 'k':
				res.user += execute_k(msg, mode, channels_list[channel]);
				break;
			case 'o':
